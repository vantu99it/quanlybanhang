    ...
    if (!isset($_SESSION['logins'])) {
        ...
        if($_SERVER["REQUEST_METHOD"] == "POST"){
            ...
            if($queryAmount->rowCount() > 0){
                ...
                $output = $resultsAmount->output; // xuất kh
                $cancel = $resultsAmount->cancel; //Hủy kho
                $sold = $resultsAmount->sold; // đã bán
                $remaining = $resultsAmount->remaining; // còn lại kệ

                // Tạo thông tin mới
                $cancelNew = $cancel + $quantity;
                $remainingNew = $output - $cancelNew - $sold ;

                $queryAmount= $conn -> prepare("UPDATE tbl_amount SET cancel = :cancel, remaining = :remaining, update_at = now() WHERE id_product = :id_product AND id_brand = :id_brand");
                $queryAmount->bindParam(':cancel',$cancelNew,PDO::PARAM_STR);
