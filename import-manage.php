    ...
    if (!isset($_SESSION['logins'])) {
        ...
        if(isset($_GET['brand'])){
            if($_SERVER["REQUEST_METHOD"] == "POST"){
                ...
                $id_brand = $_GET['brand'];
                $queryWare= $conn -> prepare("SELECT wa.*, us.username AS user, pro.name AS product, br.name AS brand FROM tbl_warehouse wa JOIN tbl_product pro ON pro.id = wa.id_product JOIN tbl_user us ON us.id = wa.id_user JOIN tbl_brand br ON br.id = wa.id_brand WHERE wa.id_act = 1 AND wa.id_brand = :id_brand AND wa.created_ad <= :toDate AND wa.created_ad >= :fromDate ORDER BY wa.created_ad DESC");
                ...
                $id_brand = $_GET['brand'];
                $queryWare= $conn -> prepare("SELECT wa.*, us.username AS user, pro.name AS product, br.name AS brand FROM tbl_warehouse wa JOIN tbl_product pro ON pro.id = wa.id_product JOIN tbl_user us ON us.id = wa.id_user JOIN tbl_brand br ON br.id = wa.id_brand WHERE wa.id_act = 1 AND wa.id_brand = :id_brand AND wa.created_ad <= :today AND wa.created_ad >= :previous_date ORDER BY wa.created_ad DESC");
                $queryWare->bindParam('id_brand',$id_brand,PDO::PARAM_STR);
            ...
            if($_SERVER["REQUEST_METHOD"] == "POST"){
                ...
                $queryWare= $conn -> prepare("SELECT wa.*, us.username AS user, pro.name AS product, br.name AS brand FROM tbl_warehouse wa JOIN tbl_product pro ON pro.id = wa.id_product JOIN tbl_user us ON us.id = wa.id_user JOIN tbl_brand br ON br.id = wa.id_brand WHERE wa.id_act = 1 AND wa.created_ad <= :toDate AND wa.created_ad >= :fromDate ORDER BY wa.created_ad DESC");
                $queryWare->bindParam('toDate',$toDate,PDO::PARAM_STR);
                $queryWare->bindParam('fromDate',$fromDate,PDO::PARAM_STR);
                $queryWare-> execute();
                $resultsWare = $queryWare->fetchAll(PDO::FETCH_OBJ);
                ...
                $queryWare= $conn -> prepare("SELECT wa.*, us.username AS user, pro.name AS product, br.name AS brand FROM tbl_warehouse wa JOIN tbl_product pro ON pro.id = wa.id_product JOIN tbl_user us ON us.id = wa.id_user JOIN tbl_brand br ON br.id = wa.id_brand WHERE wa.id_act = 1 AND wa.created_ad <= :today AND wa.created_ad >= :previous_date ORDER BY wa.created_ad DESC");
                $queryWare->bindParam('today',$today,PDO::PARAM_STR);
                $queryWare->bindParam('previous_date',$previous_date,PDO::PARAM_STR);
                $queryWare-> execute();
                $resultsWare = $queryWare->fetchAll(PDO::FETCH_OBJ);
        ...
        <div id="main-right">
            <section class="main-right-title" style = "margin-bottom: 5px;">
                <div class="form-title">
                    <h1>QL nhập kho CS<?php echo $id_brand?></h1>
                </div>
