    ...
    if (!isset($_SESSION['logins'])) {
        ...
        if($_SERVER["REQUEST_METHOD"] == "POST"){
            ...
            if($queryAmount->rowCount() > 0){
                ...
                $outputNew = $output + $quantity;
                $remainingNew = $outputNew - $cancel - $sold ;

                $queryAmount= $conn -> prepare("UPDATE tbl_amount SET input = :input, output = :output, remaining = :remaining WHERE id_product = :id_product AND id_brand = :id_brand ");
                $queryAmount->bindParam(':output',$outputNew,PDO::PARAM_STR);
