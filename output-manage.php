<?php 
    ...
    if (!isset($_SESSION['logins'])) {
        ...
        if(isset($_GET['brand'])){
            $id_brand = $_GET['brand'];
            $queryWare= $conn -> prepare("SELECT wa.*, us.username AS user, pro.name AS product, br.name AS brand, am.remaining, am.output FROM tbl_warehouse wa JOIN tbl_product pro ON pro.id = wa.id_product JOIN tbl_user us ON us.id = wa.id_user JOIN tbl_brand br ON br.id = wa.id_brand JOIN tbl_amount am ON am.id_product = pro.id WHERE wa.id_act = 2 AND wa.id_brand = :id_brand ORDER BY wa.created_ad DESC");
            $queryWare->bindParam('id_brand',$id_brand,PDO::PARAM_STR);
            $queryWare-> execute();
            $resultsWare = $queryWare->fetchAll(PDO::FETCH_OBJ);
        }else{
            $queryWare= $conn -> prepare("SELECT wa.*, us.username AS user, pro.name AS product, br.name AS brand, am.remaining, am.output FROM tbl_warehouse wa JOIN tbl_product pro ON pro.id = wa.id_product JOIN tbl_user us ON us.id = wa.id_user JOIN tbl_brand br ON br.id = wa.id_brand JOIN tbl_amount am ON am.id_product = pro.id WHERE wa.id_act = 2 ORDER BY wa.created_ad DESC");
            $queryWare-> execute();
            $resultsWare = $queryWare->fetchAll(PDO::FETCH_OBJ);
        }
    }
